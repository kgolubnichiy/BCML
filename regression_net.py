...
class Net(nn.Module):
    def __init__(self):
        ...
        #self.hidden1 = nn.Linear(13, 100, True)
        #self.hidden2 = nn.Linear(100, 50, True)
        #self.hidden3 = nn.Linear(50, 25, True)
        #self.output = nn.Linear(25, 1, True)
        self.to(get_device())
...
def print_hist(y_pred, y, r, norms):
    size = len(y)
    pbins = [0]*11
    tpbins = [0]*11
    #print(y_pred)
    #print(y)
    with torch.no_grad():
        y_ml = torch.heaviside(y_pred - r, torch.tensor(1.0).expand_as(y_pred))
        y_lb = torch.heaviside(y - r, torch.tensor(1.0).expand_as(y_pred))
        tp = torch.logical_and(y_ml, y_lb)
        #print(tp)
        for i in range(size):
            if y_ml[i]:
...
def calc_acc(y_pred, y, r):
    ...
    with torch.no_grad():
        ...
        y_ml = torch.heaviside(y_pred - r, torch.tensor(1.0).expand_as(y_pred))
        y_lb = torch.heaviside(y - r, torch.tensor(1.0).expand_as(y_pred))
        tp = torch.sum(torch.logical_and(y_ml, y_lb))
        p_pred = torch.sum(y_ml)
        p_label = torch.sum(y_lb)
        #print(y_th)
        #print(torch.logical_xor(y_th, y))
        acc = float(size - torch.sum(torch.logical_xor(y_ml, y_lb)))*100/size
...
if __name__ == '__main__':
    ...
    for i in range(len(input_df)):
        ...
        #label
        real0 = (input_df[i][6] + input_df[i][9])/2 - 0.000000001
        label = 1 if input_df[i][17] >= real0 else 0
        tot_pos += label
        #if label != int(input_df[i][23]):
        #    print(input_df[i][0] + ": " + str(input_df[i][17]) + " ? " + str(real0) + "=" + str(input_df[i][6]) + "+" + str(input_df[i][9]) + "/2 " + str(label))
        forecast = 1 if input_df[i][15] >= real0 else 0
        ...
        input_df[i][17] = (input_df[i][17] - av)/sigma
        input_df[i][18] = (real0 - av)/sigma
        #print(input_df[i])
    ...
    r = torch.from_numpy(input_df[0:size_train, 18:19].astype(float)).float().to(device)
    #print(y.shape)
    #print(y)
    #print(torch.sum(y))
    ...
    #print(nnn)
    optimizer = torch.optim.Adam(nnn.parameters(), lr=0.0001, weight_decay=0.00001)
    ...
    for t in range(10000):
        ...
        if t % 1000 == 0:
            ...
            [ml_acc, ml_prec, ml_rec] = calc_acc(y_pred, y, r)
    ...
    for i in range(len(y)):
      ...
      if l > 10000:
        opt = input_df[i][0]
        print(f"{i} {opt} pred {pred} real {real}")
    ...
    inp_pos = float(tot_pos)*100/size_train
    ...
    print_hist(y_pred, y, r, input_df[:, 2])
    #print_hist(torch.from_numpy(input_df[0:size_train, 15:16].astype(float)).float().to(device), y, r, input_df[:, 2])
    if train:
        torch.save(nnn, "./nnn/regmodel1")
    #print(y_pred)
    exit(0)
    result = nnn(X_test)
